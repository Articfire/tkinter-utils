...
class FileSelectorForm(Form):
    def __init__(self, title:str, commands):
        super().__init__(title, "380x105")
        self.file_name = StringVar()

        self.label_subtitle = Label(master=self.base, text="Selecciona el archivo a procesar", width=26, font=("Verdana", 15, "normal"))
        self.label_subtitle.place(x=0, y=0)
        ...
        self.button_process = Button(self.base, text='Procesar', width=15, fg='black')
        # self.button_process.config(command=self.GetFileName)
        self.button_process.place(x=130, y=70)

        self.button_close = Button(self.base, text='Cerrar', width=15, fg='black')
...
form = FileSelectorForm("Prueba", None)
